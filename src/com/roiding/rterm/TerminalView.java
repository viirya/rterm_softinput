...
public class TerminalView extends View implements VDUDisplay {
	...
	public static final int SCREEN_HEIGHT_DEFAULT = 320;

	public int SCREEN_WIDTH = SCREEN_WIDTH_DEFAULT;
	public int SCREEN_HEIGHT = SCREEN_HEIGHT_DEFAULT;

	public float CHAR_WIDTH = SCREEN_WIDTH / TERM_WIDTH;
	public float CHAR_HEIGHT = SCREEN_HEIGHT / TERM_HEIGHT;
	...
	private float cursor_y = -1;

	private static final KeyCharacterMap DEFAULT_KEYMAP = KeyCharacterMap
	...
	public TerminalView(Context context, AttributeSet attrs,
		...
		setOnFocusChangeListener (new View.OnFocusChangeListener () {
			public void onFocusChange(View v, boolean hasFocus) {
				Log.d("TerminalView", "onFocusChange");
			}
		});
		init();
	}
	...
	public void init() {
		...
		if (!setBitmap()) {
			Toast.makeText(this.parentContext, R.string.terminal_view_outofmemory, Toast.LENGTH_SHORT).show();
                }
	...
	private void setFontSize(float size) {
		...
		if (size <= 11f) {
			CHAR_WIDTH = 6;
			CHAR_HEIGHT = 13;
			SCREEN_WIDTH = SCREEN_WIDTH_DEFAULT + 200;
			SCREEN_HEIGHT = SCREEN_HEIGHT_DEFAULT + 200;
		} else if (size <= 12.5f) {
			CHAR_WIDTH = 6.1f;
			CHAR_HEIGHT = 14;
			SCREEN_WIDTH = (int) (CHAR_WIDTH * TERM_WIDTH) + 200;
			SCREEN_HEIGHT = (int) (CHAR_HEIGHT * TERM_HEIGHT) + 200;
	...
	private boolean setBitmap() {
		...
		try {
			...
			bitmap = Bitmap.createBitmap(SCREEN_WIDTH, SCREEN_HEIGHT, Config.ARGB_8888);
	...
	public void onDraw(Canvas canvas) {
		...
		if (this.buffer.isCursorVisible()) {
			...
			cursor_x = this.buffer.getCursorColumn() * CHAR_WIDTH;
			cursor_y = (this.buffer.getCursorRow() + this.buffer.screenBase - this.buffer.windowBase)
					* CHAR_HEIGHT;
			canvas.drawRect(cursor_x, cursor_y, cursor_x + CHAR_WIDTH, cursor_y + CHAR_HEIGHT, cursorPaint);

			terminalActivity.scroll((int) cursor_x, (int) cursor_y);
	...
	public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
		outAttrs.imeOptions |= EditorInfo.IME_FLAG_NO_EXTRACT_UI | EditorInfo.IME_FLAG_NO_ENTER_ACTION | EditorInfo.IME_ACTION_NONE;
		Log.d("TerminalView", "onCreateInputConnection");
	...
	private class TermInputConnection extends BaseInputConnection {
		...
		public boolean performEditorAction(int actionCode) {
			if (actionCode == EditorInfo.IME_ACTION_DONE) {
				Log.d("performEditorAction", "done");
			}
	...
	public boolean onKeyMultiple(int keyCode, int repeatCount, KeyEvent event) {
		Log.i(TAG, "onKeyMultiple:" + keyCode);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		try {
			...
			case KeyEvent.KEYCODE_ENTER:
				((vt320) buffer).keyTyped(vt320.KEY_ENTER, ' ', event
						.getMetaState());
				return true;
			case KeyEvent.KEYCODE_DPAD_LEFT:
				((vt320) buffer).keyPressed(vt320.KEY_LEFT, ' ', event
						.getMetaState());
				return true;
			case KeyEvent.KEYCODE_DPAD_UP:
				((vt320) buffer).keyPressed(vt320.KEY_UP, ' ', event
						.getMetaState());
				return true;
			case KeyEvent.KEYCODE_DPAD_DOWN:
				((vt320) buffer).keyPressed(vt320.KEY_DOWN, ' ', event
						.getMetaState());
				return true;
			case KeyEvent.KEYCODE_DPAD_RIGHT:
				((vt320) buffer).keyPressed(vt320.KEY_RIGHT, ' ', event
						.getMetaState());
				return true;
